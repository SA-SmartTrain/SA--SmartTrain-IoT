...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Servo 1
  if (topicStr == TOPIC_ULTRASSONICO1) {                  //Condicional referente ao Servo Motor 1;
    ...
    // Servo 2
    if (topicStr == TOPIC_ULTRASSONICO2) {                  //Condicional referente ao Servo Motor 2;
      int angulo = MensagemRecebida.toInt();                //parsear ângulo enviado
      servo2.write(angulo);                                 //Movimentação;
      mqtt.publish(TOPIC_SERVO_2, String(angulo).c_str());  //Publicação do estado do Servo Motor 2;
      return;
    }
    //Movimentação dos dois Servos
    if (topicStr == TOPIC_ULTRASSONICO) {                   //Condicional referente ao Servo Motor 3;
      int angulo = MensagemRecebida.toInt();                //parsear ângulo enviado
      servo2.write(angulo);                                 //Movimentação;
      mqtt.publish(TOPIC_SERVO_2, String(angulo).c_str());  //Publicação do estado do Servo Motor 2;
      servo1.write(angulo);                                 //Movimentação;
...
void loop() {
  ...
  if (distancia > 0 && distancia < 10) {
    mqtt.publish(TOPIC_ULTRASSONICO, "120");  //enviar para o broker
  }
